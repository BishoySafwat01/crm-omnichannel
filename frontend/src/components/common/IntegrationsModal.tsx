...
import { MetaChannelsSettings } from '../settings/MetaChannelsSettings';

type IntegrationTab = 'meta_pages' | 'providers' | 'webhook';
...
export const IntegrationsModal: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<IntegrationTab>('meta_pages');
  ...
  useEffect(() => {
    if (isIntegrationsModalOpen && isAdmin) {
      loadStatus();
    }
  }, [isIntegrationsModalOpen, isAdmin]);
  ...
  if (!isAdmin) {
    return (
      <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl max-w-md w-full p-6 text-center space-y-4 shadow-2xl dir-rtl">
          <div className="w-12 h-12 rounded-2xl bg-rose-100 text-rose-600 mx-auto flex items-center justify-center">
            <Lock className="w-6 h-6" />
          </div>
          <h3 className="text-base font-extrabold text-slate-900">غير مصرح بالوصول</h3>
          <p className="text-xs text-slate-500">
            إدارة وتكوين القنوات مقتصرة حصرياً على مديري النظام (Admins & Superadmins).
          </p>
          <button
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white/95 backdrop-blur-2xl border border-white/80 shadow-2xl rounded-3xl max-w-4xl w-full p-6 space-y-5 animate-in fade-in zoom-in-95 duration-150 dir-rtl text-right my-auto max-h-[90vh] flex flex-col">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-extrabold text-slate-900">
                مركز ربط القنوات والويب هـوك (Omnichannel Hub)
              </h2>
              <p className="text-xs text-slate-500 font-medium">
                إدارة الربط المباشر مع Facebook Pages, WhatsApp Cloud & Instagram Direct
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 border-b border-slate-100 pb-2 shrink-0">
          <button
            onClick={() => setActiveTab('meta_pages')}
            className={`px-4 py-2 rounded-xl text-xs font-black transition flex items-center gap-2 ${
              activeTab === 'meta_pages'
                ? 'bg-[#1877F2] text-white shadow-md shadow-blue-500/20'
                : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
            }`}
          >
            <Facebook className="w-3.5 h-3.5 fill-current" />
            <span>صفحات فيسبوك وقنوات ميتا (Meta OAuth & Pages)</span>
          </button>
          ...
          >
            ...
            <span>المزودات السحابية (Cloud Providers)</span>
          </button>
          ...
          >
            ...
            <span>رابط الويب هـوك (Webhooks)</span>
          </button>
        </div>

        {/* Tab Content Area (Scrollable) */}
        <div className="overflow-y-auto pr-1 space-y-4 flex-1">
          {activeTab === 'meta_pages' && <MetaChannelsSettings />}

          {activeTab === 'providers' && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {/* 1. WhatsApp Cloud API Card */}
                <div className="bg-emerald-50/50 border border-emerald-200/60 rounded-2xl p-4 space-y-3 relative overflow-hidden flex flex-col justify-between">
                  <div>
                ...
                <div className="bg-pink-50/50 border border-pink-200/60 rounded-2xl p-4 space-y-3 relative overflow-hidden flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-[10px] px-2 py-0.5 rounded-full font-extrabold ${
                          statusData?.instagram?.connected
                    ...
                    <div className="space-y-1.5 text-[11px] text-slate-600 pt-3">
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold text-slate-800">
                          {statusData?.instagram?.username || '@luxira.official'}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400">Page ID:</span>
                        <span className="font-mono font-bold text-slate-800">
                          {statusData?.instagram?.page_id || 'غير متاح'}
                        </span>
                      </div>
                      <div className="flex justify-between pt-1.5 border-t border-pink-100">
                        <span className="text-slate-400">Token Status:</span>
                        <span className="font-bold text-pink-700">
                          {statusData?.instagram?.status || 'VALID'}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-blue-50/50 border border-blue-200/60 rounded-2xl p-4 space-y-3 relative overflow-hidden flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-[10px] px-2 py-0.5 rounded-full font-extrabold ${
                          statusData?.messenger?.connected
                    ...
                    <div className="space-y-1.5 text-[11px] text-slate-600 pt-3">
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold text-slate-800">
                          {statusData?.messenger?.pages?.join(' / ') || 'LAVVA / LUXIRA'}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400">Page ID:</span>
                        <span className="font-mono font-bold text-slate-800">
                          {statusData?.messenger?.page_id || 'غير متاح'}
                        </span>
                      </div>
                      <div className="flex justify-between pt-1.5 border-t border-blue-100">
                        <span className="text-slate-400">Webhook:</span>
                        <span className="font-bold text-blue-700">
                          {statusData?.messenger?.status || 'SUBSCRIBED'}
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-100 shrink-0">
          <span className="text-[11px] text-slate-400 font-medium flex items-center gap-1">
            ...
            تتم مزامنة الأحداث مباشرة عبر WebSockets & Meta Graph API
          </span>
          <button
            onClick={() => setIsIntegrationsModalOpen(false)}
